...
const seedIPL2026PlayersV2 = async () => {
    try {
        console.log('🏏 Starting IPL 2026 Player Database Creation (V2 - Format Separated)...\n');
        await connectDB();
        ...
        console.log('\nFormat-wise Statistics Sample (Top 3 Players):');
        players.slice(0, 3).forEach(p => {
            console.log(`\n${p.name} (${p.nationality}):`);
            console.log(`   Overall: ${p.stats.matches}M, ${p.stats.runs}R, ${p.stats.wickets}W`);
            console.log(`   Test: ${p.testStats.matches}M, ${p.testStats.runs}R, ${p.testStats.wickets}W`);
            console.log(`   ODI: ${p.odiStats.matches}M, ${p.odiStats.runs}R, ${p.odiStats.wickets}W`);
            console.log(`   T20I: ${p.t20iStats.matches}M, ${p.t20iStats.runs}R, ${p.t20iStats.wickets}W`);
            console.log(`   T20: ${p.t20Stats.matches}M, ${p.t20Stats.runs}R, ${p.t20Stats.wickets}W`);
            console.log(`   IPL: ${p.iplStats.matches}M, ${p.iplStats.runs}R, ${p.iplStats.wickets}W`);
        });
...
function processMatchByFormat(
    ...
    const isIPL = matchData.info?.event?.name?.toLowerCase().includes('indian premier league') || format === 'ipl';
                ...
                for (const delivery of over.deliveries || []) {
                    ...
                    if (playersMap.has(batterName)) {
                        ...
                        if (!matchBattingStats.has(batterName)) {
                            matchBattingStats.set(batterName, { runs: 0, balls: 0, fours: 0, sixes: 0 });
                        }
